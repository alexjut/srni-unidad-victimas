...
- **Respuesta a la OTI** (concepto técnico sobre Auth SSO e interoperabilidad), con el
  informe v2 en PDF adjunto: `entregables/2026-09-18-oti-arquitectura/pdf/`. Queda
  esperar respuesta a las tres definiciones pedidas —OIDC con PKCE, punto de
  publicación de llaves y vigencias compatibles con jornadas sin señal— y agendar la
  mesa técnica.
- **Los 10 correos de claves provisionales de VIVANTO.** Las cuentas quedan activas y
  las personas ya pueden entrar. La APK 1.2.8 trae la pantalla para cambiar la clave
  desde el teléfono, que es lo que faltaba para poder retirar la provisional. Los
  archivos con las claves viven fuera del repo:
  `C:\Users\millo\Documents\SICAV-claves-vivanto\`.
...
## Lo que conviene vigilar tras los envíos

- Que las 10 personas **logren entrar** y cambien su clave provisional. Si alguna no
  puede, el canal de soporte sigue sin definirse: hoy la respuesta es escribirle a
  desarrollo.
- Que la OTI responda las tres definiciones del SSO. Mientras no lleguen, no hay nada
  que construir de ese lado y la operación sigue igual.
- **LLOZANOF** sigue sin crear: está en VIVANTO pero aún sin aprobación.
