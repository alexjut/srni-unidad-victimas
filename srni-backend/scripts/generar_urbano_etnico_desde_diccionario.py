...
# -*- coding: utf-8 -*-
"""
Genera el fixture del instrumento URBANO_ETNICO V1 desde el diccionario oficial.

Mismo layout 12-col que Territorial (col [10]=ESTADO). Usa códigos estándar
(DT_ATENCION, C1, Z4, I1A1, AT1...) → derivable de Territorial. Capítulos propios:
SP (Salud y atención psicosocial, SA_PS_*). El "MOD_SAI" del diccionario es
residual (1 pregunta de observaciones) → se pliega al capítulo M (uso del territorio).

Uso:
    python scripts/generar_urbano_etnico_desde_diccionario.py            # inspección
    python scripts/generar_urbano_etnico_desde_diccionario.py --escribir # escribe fixture
    python scripts/generar_urbano_etnico_desde_diccionario.py --muestra  # + muestra
"""
...
XLSX = Path(r"D:/desarrollo/perfiles/Perfil Urbano Etnico__web/Diccionario_de_datos__Entrevista de Caracterización_Perfil Urbano EtnicoV1.xlsx")
OUT = Path(__file__).resolve().parents[1] / "apps/formulario/fixtures/perfil_urbano_etnico_v1.json"
...
CAP_MAP = {
    ...
    'J.ALIMENTACIÓN':                 ('JA', 5),
    'SALUD_ATENCION PSICOSOCIAL':     ('SP', 6),
    'J. FUERZA DE TRABAJO':           ('JF', 7),
    'K. PERFIL SOCIOLABORAL':         ('K', 8),
    'USO Y DISFRUTE DEL TERRITORIO':  ('M', 9),
    'MOD_SAI':                        ('M', 9),   # residual (observaciones) → uso del territorio
    'T. CONTROL':                     ('T', 10),
}

CAP_NOMBRE = {
    'A': 'A. IDENTIFICACIÓN', 'B': 'B. DATOS BÁSICOS', 'C': 'C. VIVIENDA',
    'D': 'D. RETORNOS Y REUBICACIONES', 'JA': 'J. ALIMENTACIÓN',
    'SP': 'SP. SALUD Y ATENCIÓN PSICOSOCIAL', 'JF': 'J. FUERZA DE TRABAJO',
    'K': 'K. PERFIL SOCIOLABORAL', 'M': 'M. USO Y DISFRUTE DEL TERRITORIO',
    'T': 'T. CONTROL',
}
...
def derivar_skip_logic(preguntas_fix):
    """Reusa reglas de Territorial (umbral texto 0.85). Caps propios (SP) sin
    reglas aquí: su flujograma es diagrama visual → revisión manual."""
    terr = json.loads(TERR_FIX.read_text(encoding='utf-8'))
    ue_text = {p['codigo_externo']: norm(p['texto']) for p in preguntas_fix}
    te_text = {p['codigo_externo']: norm(p['texto']) for p in terr['preguntas']}
    ue_caps = {p['capitulo_codigo'] for p in preguntas_fix}

    def coincide(cod):
        if cod not in ue_text:
            return False
        b = te_text.get(cod, '')
        return bool(b) and SequenceMatcher(None, ue_text[cod], b).ratio() >= 0.85
    ...
    for r in terr['reglas_skip_logic']:
        ...
        if a and not coincide(a):
            descartadas += 1; continue
        if ac and ac not in ue_caps:
            descartadas += 1; continue
        if not a and not ac:
            descartadas += 1; continue
        reglas.append(dict(r))
    print(f"  Skip-logic derivado de Territorial: {len(reglas)} reglas "
          f"({descartadas} descartadas por no coincidir en Urbano Étnico)")
    return reglas
...
def main():
    ...
    data = {
        'perfil': {'codigo': 'URBANO_ETNICO', 'nombre': 'Caracterización Urbano Étnico'},
        'instrumento_version': {'numero': 'V1', 'vigente_desde': '2023-07-14'},
        'capitulos': capitulos,
